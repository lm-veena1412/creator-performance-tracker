...
const Dashboard = () => {
  ...
  const handleViewCreator = async (creatorId) => {
    try {
      const [creatorRes, logsRes] = await Promise.all([
        axios.get(`http://localhost:5000/api/creators/${creatorId}`),
        axios.get(`http://localhost:5000/api/performance/creator/${creatorId}`)
      ]);
  ...
  const handleDeleteLog = async (logId) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/performance/${logId}`);
      fetchDashboardData();
    } catch (error) {
